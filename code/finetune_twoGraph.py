            ...
            for args.inDrop_agg in [0.]:
                ...
                for tissue, id in tissue_id_dict.items():
                    ...
                    for i in cell_lines_id:
                        with open('input/adj_pos_twoGraph_'+str(args.thresold)+'/adj_pos_' + str(i) + '.pickle', 'rb') as f:  # file是储存在硬盘中的文件名称
                            adj_pos = pickle.load(f)
                        with open('input/adj_neg_twoGraph_'+str(args.thresold)+'/adj_neg_' + str(i) + '.pickle', 'rb') as f:  # file是储存在硬盘中的文件名称
                    ...
                    for i in range(num_cell_lines):
                        ...
                        for j in range(args.kfold):
                            # args.file = 'Record/LastModel/finetune_twoGraph_'+str(args.thresold)+'/' + '%s_%s' % (cell_i, cell_name) + '.pkl'
                            args.file = 'Record/LastModel/finetune_twoGraph_3D1_' + str(args.thresold) + '/' + '%s_%s' % (cell_i, cell_name) + '.pkl'
                            feature = drug_fea1
                            if args.kfold == 1:
                ...
                print("MSE_all_mean, spearman_all_mean, pearson_all_mean, mae_all_mean, rmse_mean:{}\t{}\t{}\t{}\t{}".format(
                                np.mean(MSE_ALL), np.mean(SPE_ALL), np.mean(PEA_ALL), np.mean(MAE_ALL),
                                np.mean(RMSE_ALL)))
