...
class TransferCell(nn.Module):
    def __init__(self, nfeat, nhid, init, dropout, dhid1, similarity, num_cell_lines,
                 cell_line_list, cell_lines_id, now_id, ndim1, ndim2, inDrop, Drop, inDrop_agg, Drop_agg, fold, thresold):
        super(TransferCell, self).__init__()
        self.now_id = now_id
        # self.similarity = similarity
        self.attW = nn.Parameter(torch.empty(len(similarity)))
        nn.init.normal_(self.attW)
        # self.attW = nn.Parameter(torch.ones(len(similarity)))
        ...
        if self.training is True:
            ...
            # file = 'Record/LastModel/preTrain_twoGraph_test_'+str(thresold)+'/' + '%s_%s' % (d, cell_line_list[d]) + '.pkl'
            file = 'Record/LastModel/preTrain_twoGraph_3D1_' + str(thresold) + '/' + '%s_%s' % (d, cell_line_list[d]) + '.pkl'
    ...
    def calSubView(self, x, cell_line_adjs_pos, cell_line_adjs_neg):
        ...
        for v in range(self.num_cell_lines):
            if v != self.now_id and isFlag:
                subview = self.similarity[v] * self.subView[t](x, cell_line_adjs_pos[v], cell_line_adjs_neg[v])

                isFlag = False
                t += 1
            elif v != self.now_id:
                temp = self.similarity[v] * self.subView[t](x, cell_line_adjs_pos[v], cell_line_adjs_neg[v])
                subview = torch.cat([subview, temp], dim=1)
                t += 1
        return subview

    def forward(self, x, cell_line_adjs_pos, cell_line_adjs_neg):
        # 开始主视角的feature提取
        main_embed = self.mainview(x, cell_line_adjs_pos[self.now_id], cell_line_adjs_neg[self.now_id])

        # 开始辅助视角的feature提取
        subview_embed = self.calSubView(x, cell_line_adjs_pos, cell_line_adjs_neg)
        # 将多个子视角feature融合
