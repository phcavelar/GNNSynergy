...
from model_dynamic_twoGraph import *
...
def main():
    ...
    for tissue, id in tissue_id_dict.items():
        ...
        for i in cell_lines_id:
            with open('input/adj_pos_twoGraph_' + str(args.thresold) + '/adj_pos_' + str(i) + '.pickle',
                      'rb') as f:  # file是储存在硬盘中的文件名称
                adj_pos = pickle.load(f)
            with open('input/adj_neg_twoGraph_' + str(args.thresold) + '/adj_neg_' + str(i) + '.pickle',
                      'rb') as f:  # file是储存在硬盘中的文件名称
        ...
        for i in range(num_cell_lines):
            ...
            train_ind_kfold = train_ind_kfold + val_ind_kfold + test_ind_kfold
            mse_kfold = []
            spearman_kfold = []
            pearson_kfold = []
            r2_kfold = []
            ...
            for j in range(args.kfold):
                # args.file = 'Record/LastModel/finetune/' + str(j) + '/%s_%s' % (cell_i, cell_name) + '.pkl'
                ...
                # 模型和优化器定义
                # 加载模型
                model = TransferCell(nfeat=drug_fea1.shape[1],
                                     nhid=args.hidden,
                                     init='Kaiming',
                ...
                if torch.cuda.is_available():  # tensor和变量转到gpu
                    ...
                    drug_fea1 = torch.FloatTensor(drug_fea1).cuda()
                ...
                if (args.train_test_mode == 1):
                    ...
                    for epoch in range(args.epochs):
                        ...
                        y_pred = model(drug_fea1, cell_line_adjs_pos, cell_line_adjs_neg)
                    ...
                    #     with torch.no_grad():
                    #         model.eval()
                    #         y_pred = model(drug_fea1, cell_line_adjs_pos, cell_line_adjs_add, cell_line_adjs_neg)
                ...
                with torch.no_grad():
                    model1.eval()
                    y_pred = model1(drug_fea1, cell_line_adjs_pos, cell_line_adjs_neg)
                    ...
                    y_pred1 = np.array(y_pred.cpu()) * ((np.array(train_mask.cpu()) * -1) + 1)
                    np.savetxt('Record/LastModel/case_study_predNovel/%s' % cell_i + '.txt', y_pred1)
